...
#문1) 다음과 같이 결측값이 포함된 데이터셋 ds를 생성한 후, 다음 문제를 해결하기 위한 R 코드를 작성하시오.
...
#문2) 다음과 같이 state.x77 데이터셋을 st에 저장한 후 다음 문제를 해결하기 위한 R 코드를 작성하시오.
...
out.Inc <- boxplot.stats(st$Income)$out
st$Income[st$Income %in% out.Inc] <- NA
...
#문3)R에서 제공하는 airquality 데이터셋에 대해 다음의 문제를 해결하기 위한 R 코드를 작성하시오.
...
AQ2 <- AQ
AQ2$Ozone[is.na(AQ2$Ozone)] <- mean(AQ2$Ozone, na.rm = T)
AQ2$Solar.R[is.na(AQ2$Solar.R)] <- mean(AQ2$Solar.R, na.rm = T)
AQ2
#확인
sum(is.na(AQ2))

#문4) R에서 제공하는 state.x77 데이터셋에 대하여 다음 문제를 해결하기 위한 R 코드를 작성하시오.

#(1) state.x77 데이터셋을 Population(인구수)를 기준으로 오름차순 정렬하시오.
library(dplyr)
st <- data.frame(state.x77, names=T)
st
st %>% arrange(Population)
#arrange는 rowname이 손실됨
...
rownames(st[order(st$Illiteracy),][1:10,])
st[order(st$Illiteracy),][1:10,"Illiteracy"]

st %>%
  mutate( st.name = rownames(st) ) %>% 
  arrange( Illiteracy ) %>% 
  select( st.name, Illiteracy ) %>% 
  head( 10 )
...
mtcars %>% filter(wt > 3.0 & wt < 5.0) #첫번째
subset(mtcars, mtcars$wt > 3.0 & mtcars$wt < 5.0 ) #두번쨰

#문6) 다음의 문제를 해결하기 위한 R코드를 작성하시오.
...
library(dplyr)
inner_join(authors, books, by=c("surname"="name")) #첫번째 방법
#오류가 떠서 위에 데이터프레임에 stringsAsFactors = F 추가함
str(authors)
str(books)
merge(authors, books, by.x=c("surname"), by.y=c("name")) #두번째 방법
...
full_join(authors, books, by=c("surname"="other.author"))
